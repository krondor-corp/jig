...
### Bug Fixes
 ...
 - <csr-id-f6ad5ff8bc202fd789f1793a5f15a9dda91824ae/> remove a worktree whose branch was renamed
 ...
 - <csr-id-71490d37b0b35a64c6fd84ddecd20bd659d8eed3/> ignore worktrees jig didn't create instead of panicking
 - <csr-id-5a2edd99a34f789d636df51d0ef849689c8657f3/> time out Linear requests so auto-spawn can't wedge
   ureq 3 has no timeouts by default. The spawn and triage actors call Linear
   on every poll, so a connection that dies without a reset (typically across
   laptop sleep) blocks `handle` forever. `ActorHandle::send` drops requests
   while one is in flight, so from then on auto-spawn silently never runs
   again until the daemon is restarted.
   
   Bound each request at 30s (10s to connect). A timeout just skips that
   poll; the next one retries.
   
   Adds a test against a local server that accepts and never answers: it
   fails in ~0.3s with the timeout and hangs indefinitely without it.
...
### Test
 ...
 - <csr-id-a4efc414454738c779ce299fa16684eaaffee362/> delete the leftover fixture in jig-core src
   `git/test_repo.rs` was a fixture living in src, duplicating the
   `seeded_repo` helper in tests/common — and it existed only for two tests
   of the private `find_valid_start_point`.
   
   Both assertions are observable through `create_worktree`: a base that
   resolves nowhere is reported missing, a local base is a valid start
   point. Those move to tests/git_repo.rs, and the fixture and its
   `#[cfg(test)]` module are gone. jig-core src now holds no test setup at
   all.
 ...
 - <csr-id-abb3aa68674fb39f4678e0402e5b7a0a9becc9d7/> compare resolved paths in foreign worktree test
   git2 reports /private/var on macOS where the tempdir says /var.
 ...
 * **Uncategorized**
    ...
    - Merge pull request #82 from krondor-corp/fix/remove-renamed-branch ([`ef80e80`](https://github.com/krondor-corp/jig/commit/ef80e800e4c42ae32c1e76c0452f50fb56d9a1cd))
    - Remove a worktree whose branch was renamed ([`f6ad5ff`](https://github.com/krondor-corp/jig/commit/f6ad5ff8bc202fd789f1793a5f15a9dda91824ae))
    ...
    - Delete the leftover fixture in jig-core src ([`a4efc41`](https://github.com/krondor-corp/jig/commit/a4efc414454738c779ce299fa16684eaaffee362))
    ...
    - Merge pull request #66 from krondor-corp/fix/pending-review-nudges ([`f57a774`](https://github.com/krondor-corp/jig/commit/f57a774eb383dac861766d3d6a49b777ef61563f))
    ...
    - Compare resolved paths in foreign worktree test ([`abb3aa6`](https://github.com/krondor-corp/jig/commit/abb3aa68674fb39f4678e0402e5b7a0a9becc9d7))
    ...
    - Ignore worktrees jig didn't create instead of panicking ([`71490d3`](https://github.com/krondor-corp/jig/commit/71490d37b0b35a64c6fd84ddecd20bd659d8eed3))
    - Merge pull request #59 from krondor-corp/fix/linear-http-timeouts ([`ba918c2`](https://github.com/krondor-corp/jig/commit/ba918c2e8484309f03ac6b390869f92d8df454fd))
    - Time out Linear requests so auto-spawn can't wedge ([`5a2edd9`](https://github.com/krondor-corp/jig/commit/5a2edd99a34f789d636df51d0ef849689c8657f3))
