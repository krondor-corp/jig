...
### Chore
 ...
 - <csr-id-0be92fcc3329a14892f0fce4aa49de59a4e3fbdf/> bump version to 0.9.1
 - <csr-id-af1abaf595b26f3d8dc08c61f352dee4f727c318/> bump version to 0.9.0
 - <csr-id-6b4aa31f5c42e77db93ad73ec7e3f40a798dc914/> bump version to 0.8.0
 - <csr-id-ef249cba46122dda853f603e5a7851a4e20cef43/> bump version to 0.7.0
 - <csr-id-b03b2c10f48ca83266267c53d0d24e51aa6944ec/> bump version to 0.6.0
...
### New Features
 ...
 - <csr-id-01ba21482b0a5c28af9b8a1e572b03c9e6c5752a/> jig daemon install --as <user> (and --env)
   On a server the person with sudo is not the person the daemon runs as.
   Install took the target user from SUDO_USER, so `sudo jig daemon install`
   run by an admin — or by ansible's become — silently created a service
   running as the admin, discovered later when the daemon could not see the
   service account's repos.
   
   `--as <user>` names them. Everything then comes from that user: their
   groups (via User= and initgroups), their home, their uid for the runtime
   directory that holds the socket. SUDO_USER stays as the default, which is
   right when installing for yourself.
   
   The environment follows the same rule. Installing for yourself keeps your
   PATH / XDG_CONFIG_HOME / SSH_AUTH_SOCK; installing for someone else those
   are root's, not theirs, so the service gets a PATH rooted at the target's
   home and anything further comes from `--env KEY=VALUE`.
   
   An ansible play is now one task with no environment block:
   
       - ansible.builtin.command: /usr/local/bin/jig daemon install --as bot
         args: { creates: /etc/systemd/system/jig-daemon.service }
         become: true
 - <csr-id-d6c579d770d77c7a5a2d4bea0c6799d2cfc0d94d/> install as a system service that runs as you (Linux)
   The daemon runs your hooks, so it needs your permissions. A
   `systemd --user` service cannot have them: the user manager is started by
   PID 1 with your uid and primary group and no privilege to add the rest,
   so `docker` and every other supplementary group is missing — which
   surfaces much later as
   
       auto-spawn failed: hook failed: permission denied while trying to
       connect to the docker API at unix:///var/run/docker.sock
   
   and no amount of relogging fixes it.
   
   `jig daemon install` on Linux now writes a *system* unit with
   `User=<installer>`; systemd runs initgroups for it, so the daemon has
   exactly the groups the person who installed it has. Not root? It says so
   and names both ways forward, rather than installing something subtly
   broken. `--user` still installs the user-level service, with a warning
   about the group limit. macOS is unchanged: launchd agents already carry
   the user's groups.
   
   The unit is written by hand (the crate's template can't express it):
   `Requires=user-runtime-dir@<uid>.service` so the socket's directory
   exists before the daemon starts at boot, `HOME` and `XDG_RUNTIME_DIR` for
   the target user so `jig daemon status` from their shell finds the same
   socket, plus PATH / XDG_CONFIG_HOME / SSH_AUTH_SOCK carried over — the
   last one being what `git fetch` over SSH asks for a key.
   
   Documented in the monitoring wiki, including the docker symptom.
 - <csr-id-1a4364ca9ab2b1f64e00f4e982c349f7dde0a992/> jig daemon install | uninstall (service plan phase 5)
   `jig daemon start` only runs in the foreground, so keeping the daemon up
   meant nohup or a hand-written unit file.
   
   `jig daemon install` registers it with the OS through `service-manager`:
   systemd `--user` on Linux, a launchd user agent on macOS. The service
   runs `jig daemon start` — the daemon itself is unchanged — with autostart
   at login and restart-on-failure, which is the supervisor KRO-213 wants
   for panics on the tick thread and kills the process can't catch.
   
   It carries over `PATH` (jig shells out to git, gh and the mux) and
   `XDG_CONFIG_HOME` when set, so the service reads the same config as the
   shell that installed it. On Linux it points out `loginctl enable-linger`,
   without which a user service dies at logout — the case on a box you only
   ssh into.
   
   `jig daemon uninstall` stops it if running, then removes it.
 - <csr-id-35f39e94ac04fb8118782a9d84a08c9471328915/> make the --once monitor wait configurable
   The 30s bound on how long `daemon start --once` waits for its monitor
   pass was a hardcoded constant, and a pass polling many repos over a slow
   network can outrun it — leaving a tick that looks complete but is not.
   
   `--timeout <SECONDS>` raises it, and a run that hits the bound now says
   so instead of exiting quietly. The flag `requires = "once"`, since it
   bounds nothing in a looping daemon; it takes `Option<u64>` rather than a
   clap default because clap counts a defaulted argument as present, which
   would make that requirement reject every ordinary `daemon start`.
   
   The wait itself moves to `Daemon::wait_for_monitor`, next to the actor it
   waits on — `jig ps` had its own copy of the same loop and now shares it.
 - <csr-id-163d30cf9748f798dd646fc5064f41fd2a1e15ea/> render from the running daemon, falling back in-process
   `jig ps` and `ps --watch` are IPC clients: one round trip instead of a
   tick loop of their own, so several dashboards can be open at once and a
   one-shot `jig ps` no longer spins up a daemon for a tick. The daemon is
   always global, so a repo-scoped `ps` narrows the frame it gets back
   rather than asking a narrower question.
   
   With no daemon listening, both fall back to driving one in-process —
   what `jig ps` always did. A *global* `ps --watch` also takes the socket
   and PID file while it does, so `jig daemon status` and other `ps`
   invocations can see it; a repo-scoped one does not, since answering for
   one repo under the name of a daemon that watches all of them would be a
   lie. The header says which of the three is happening.
   
   A daemon that answers badly — one left over from an older jig — logs a
   warning and falls back rather than failing the command.
 - <csr-id-a93a37535a7f959654b901b501eff250294cfee8/> jig daemon start | stop | status over the socket
   `start` is the daemon's own command now, instead of it living inside
   `jig ps -gw`: it claims the PID file, binds the socket, installs
   SIGINT/SIGTERM handlers so an interrupted daemon still unlinks both, and
   runs the tick loop. `--once` runs a single tick (waiting out the monitor
   pass) and exits.
   
   `status` moves from reading the heartbeat file to `Request::Ping` — a
   reply *is* the proof of life, where the file could only approximate it.
   It keeps the per-actor busy/finished lines, now carried in the response.
   When nothing answers but the PID file still names a live process, say so
   rather than "not running", which would send the user to start a rival.
   
   `stop` asks over IPC and reports the PID it stopped; a connection dropped
   mid-shutdown is the shutdown working, not a failure.
 - <csr-id-a59fb0634e2b80d116bb26e49e3637427287eeb5/> bind a unix socket and enforce a single instance
   The daemon only existed inside `jig ps -gw`, so "is it running, is it
   healthy, what is it doing" had no owner to ask — the Sep 8 stall went
   unnoticed for two days, and any number of `ps -gw` instances could run at
   once fighting over the same worktrees.
   
   Now it answers for itself. `Server` binds `daemon.sock` and serves
   newline-delimited JSON: `Ping`, `GetStatus`, `Shutdown`. `PidFile` is the
   single-instance gate — a second daemon fails with the PID of the first,
   and a PID whose process is gone is a stale claim the next start takes
   over, so a crash leaves nothing to clean up by hand. A socket file left
   by a crashed daemon is detected the same way: connecting to it fails, so
   it is stale and gets rebound.
   
   The listener reads `DaemonShared` — the actor `Arc`s plus atomics for
   `ticked_at` and the poll deadline — rather than the `Daemon` itself, so a
   status request can never block a tick, nor a tick a status request. That
   also means a stalled daemon still replies, and `ticked_at` in the reply
   is what makes the stall visible.
   
   This subsumes `daemon-heartbeat.json` from #57: the PID file does the
   liveness half, and the per-actor busy/finished times it carried now ride
   in `DaemonInfo`.
 - <csr-id-d67979702998c1e9609794af82afdbe6834a1173/> wire types for IPC — serde derives and socket paths
   `MuxStatus`, `PrHealth`, `PrChecks`, `AgentState`, `RepoEntry` and
   `TriageEntry` all cross the daemon socket, so they need to serialize.
   
   Socket and PID file live in `$XDG_RUNTIME_DIR/jig/` when the session has
   one: user-private, on tmpfs, and cleared on logout, so a reboot can never
   leave a stale pair behind. Falls back to `~/.config/jig/state/` where
   there is no runtime dir (macOS).
 - <csr-id-0365d6ccd806f92a36b54a860b27438704843291/> flag a failed GitHub check in the HEALTH column
   `pr_error` was collected and never displayed. A failed check showed `?`
   at best, and a bare `-` when PR discovery itself failed — which is the
   first thing to fail when `gh` is broken, and reads as "nothing wrong
   here". The column now shows `? gh` whenever a check errored, whether or
   not a PR was found; the reason stays in `jig daemon logs` rather than
   widening the table.
 - <csr-id-b598304239ed82eff3c08dea9fea43e0ff6b9963/> add `jig daemon status` and `jig daemon logs`
   The daemon lives inside `jig ps -w`, so there was no reliable way to tell
   a healthy daemon from a dead or wedged one: every jig command created its
   own (almost always empty) log file, one-shot `jig ps` wrote Started/Stopped
   pairs to daemon.jsonl, and a healthy tick logs nothing.
   
   - The long-running daemon rewrites state/daemon-heartbeat.json every tick:
     pid, version, last tick, log path, and per-actor busy/finished times.
     A hung actor (e.g. a fetch stuck on auth) is now visible even though the
     tick loop keeps going.
   - `jig daemon status` (default for `jig daemon`) reports running / stalled /
     not running, flags actors busy >10m, and exits non-zero unless healthy.
   - `jig daemon logs [-f] [-n N] [--path]` prints the daemon's own log and
     follows it across restarts.
   - Daemon runs log to `<ts>-daemon.log`; other commands only create a log
     file if they actually write to it.
   - One-shot `jig ps` uses `Daemon::oneshot`, which skips the lifecycle log
     and heartbeat.
   - The shell wrapper passes `daemon` straight through so `logs -f` streams
     instead of being captured.
   
   Stands in for the pidfile of docs/daemon-service-plan.md until the daemon
   binds a socket.
...
### Bug Fixes
 ...
 - <csr-id-612ea5e864a4dd5f23021d4158bca2cc09e911ad/> look up PRs by the checked-out branch (KRO-217)
   The monitor asked GitHub about `worker.branch()`, which despite its name is
   the worker's folder under `.jig/`, not the branch checked out in it. The two
   are identical until someone renames the branch, after which the PR is never
   found: the worker never reaches Merged or Failed, so it is never pruned,
   never PR-nudged, and never auto-completed. Two such worktrees are sitting in
   this repo with merged PRs.
   
   Ask the worktree what is actually checked out. `Worktree::checked_out_branch`
   returns that, or `None` on a detached HEAD — where there is no branch to look
   up and the PR check is skipped rather than guessed at. The folder name stays
   the worker's identity for the event log, mux window and prune target.
   
   `check_ci` takes its ref from `check_pr`, so it is fixed by the same change.
 - <csr-id-93435fb70ed5e6d53babf57686828e0f19aa89c7/> stop unit tests mutating the process environment
   Tests set XDG_CONFIG_HOME / XDG_RUNTIME_DIR (and one the working
   directory) process-wide. Each guarded itself with a private lock at most,
   so they raced each other and any test resolving a jig path — and
   `paths.rs` removed XDG_CONFIG_HOME afterwards, letting concurrent tests
   write into the developer's real ~/.config/jig.
   
   Take paths instead of reading the environment in the code under test:
   - `PidFile::acquire_at(path)`; `acquire()` passes the default
   - `RepoRegistry::load_from` / `save_to`
   - `Context::for_repo(repo, config, registry_file)`, split out of
     `from_cwd`, so its test needs neither env nor `set_current_dir`
   - `paths.rs`: one `xdg_jig_dir` helper for XDG_CONFIG_HOME and
     XDG_RUNTIME_DIR, with a pure `jig_dir_from` the tests call directly.
     `global_config_dir` now treats an empty XDG_CONFIG_HOME as unset, as
     the XDG spec says, instead of resolving a relative `jig/`.
   
   No `env::set_var`, `remove_var` or `set_current_dir` remains in either
   crate; the full suite passes 10/10 at --test-threads=16.
 - <csr-id-5112660e9ea51467100c51a3907b299ee0663469/> ignore draft review comments; cap nudges at 3 then notify
   The daemon's `gh` is usually logged in as the reviewer, and GitHub shows a
   reviewer their own unsubmitted ("Pending") review comments. Review threads,
   the REST fallback, and the push-after-feedback check all counted those
   drafts as feedback, so a worker was nudged — and "fixed" things — while
   the reviewer was still writing the review. Verified against PR #64: both
   threads the worker kept being nudged about came back `PENDING`.
   
   Filter drafts everywhere feedback is read: the GraphQL thread comment
   `state`, and `pull_request_review_id` against PENDING review ids for REST.
   
   Separately, nothing capped nudges despite the docs promising escalation
   after `max_nudges`; the worker on PR #64 reached nudge 16, one every 5m.
   After 3 nudges of a type the daemon now stops and emits a single
   NeedsIntervention notification. The limit is fixed, not config.
 - <csr-id-9c01e174376fb635e4e235d826fbeee93dbe55cb/> say when a one-shot table may be incomplete
   `ps` waits for the monitor pass before reading worker state, and was
   dropping the flag that says the wait timed out — printing a table missing
   whatever the pass had not reached yet, with nothing to distinguish it
   from a complete one.
 - <csr-id-e389909ef79c2f595295c0fd5d63e3e85b39ecfe/> don't let a watching client shadow the daemon's log
   `hosts_daemon()` was two same-named methods on two `Command` enums, and
   the outer one guessed: it claimed any `ps --watch` hosts a daemon. Since
   this PR that is false — `ps --watch` against a running daemon is a plain
   client.
   
   The guess had teeth. The predicate picks the log filename, and a
   `-daemon.log` is a claim `jig daemon logs` trusts later: with no daemon
   up to name its own log, it takes the newest one. So `jig ps -gw` run as a
   client left an empty `-daemon.log` that shadowed the real daemon's the
   moment that daemon exited — `jig daemon logs` showed nothing, for the
   stall-debugging case this whole PR exists to serve.
   
   Collapsed to one predicate, `runs_daemon_loop()`, that answers honestly:
   `daemon start` always runs the loop, and `ps --watch` only when no daemon
   is already listening — which the parsed arguments cannot tell us, so it
   asks over the socket. The probe is injectable, so the decision is tested
   without a live socket.
   
   Both tests fail without the fix.
 - <csr-id-5730e49273805cefa260837789cb7b52193e78d6/> lock down the socket directory and stop the key reader
   Two things review turned up:
   
   The `~/.config/jig/state` fallback is not 0700 the way `$XDG_RUNTIME_DIR`
   is, and tightening only the socket leaves a window between `bind` and
   `set_permissions` where it is world-connectable. Lock the directory, which
   is the real gate.
 - <csr-id-ed65673abbccd09786a5a2f943ca0a8e3dbebedf/> report GitHub failures instead of swallowing them
   PR tracking could stop working with nothing to show for it.
   `GitHubClient::from_repo_path(..).ok()` turned a missing or logged-out
   `gh` into `None` and skipped the whole PR check without a log line, and
   PR discovery errors were logged at `debug!` — below the `info` threshold
   the daemon's log file keeps.
   
   A failed client build now sets `pr_error` like any other failed check,
   and the monitor warns when a worker's `pr_error` differs from the
   previous tick's. Comparing against the last tick rather than the state
   means a `gh` that stays broken says its piece once instead of every
   poll, and a recovery is noted when it clears.
   
   Two related fixes:
   - `pr_error` is cleared on a successful check, not only set on a failing
     one. `pr_health` carries over between ticks, so a stale error would
     otherwise stick in `jig ps` long after GitHub started answering.
   - A failed poll marks the worker polled, so a broken `gh` backs off to
     the 60s interval instead of re-running `gh repo view` every tick.
     Terminal workers are skipped for the same reason: nothing acts on the
     result.
   
   `check_pr`'s `debug!` on discovery failure is dropped — the error is
   returned and warned about upstream, so logging it here too reported the
   same failure twice at two levels. That left `worker_key` unused, so it
   is gone from the signature.
 - <csr-id-1c4ae48276f0f01787ede1d774112bb501dbe939/> claim `pending` before dispatch so actors can't wedge
 - <csr-id-9575e09791a3cc4a7c039e2720c518eb09c15345/> say when non-jig worktrees are skipped; --all shows them again
   Skipping worktrees jig didn't create was silent, so e.g. a Claude Code
   worktree just vanished from `jig ls`. Print a dim note after the table
   naming what was left out (table output only — `-p` stays script-clean).
   
   `jig ls --all` promises every git worktree but went through the now
   jig-only `list_worktrees`; it uses `Repo::linked_worktree_paths` instead.
 - <csr-id-46c884c215ca734207a1970799208ee119e2e69c/> keep actors alive when a request panics
   A panic in an actor's `handle` killed its thread. `pending` was never
   cleared, so `ActorHandle::send` dropped every later request and the actor
   (spawn, triage, monitor, ...) silently never ran again until the daemon
   restarted. The panic message went to stderr underneath the `ps -w` TUI,
   so nothing was left in the log.
   
   Catch the unwind, log it at error, clear pending, and keep serving
   requests. The test hangs in "actor stuck pending" without this change.
 - <csr-id-24dd9999d13ad949a32286d5c0c20526d8563ac7/> authenticate the background fetch
   The sync actor's `git fetch` passed `None` for FetchOptions, so libgit2 had
   no credentials callback and every fetch against an authenticated remote
   failed with "authentication required but no callback set". The daemon has
   therefore never refreshed remote-tracking refs for SSH remotes — worktrees
   were created from whatever `origin/<base>` was at the last manual fetch.
   
   The only credentials closure in the codebase was inlined in `push_branch`.
   Extract it to a module-level `remote_callbacks()` and route both `fetch` and
   `push_branch` through it.
   
   Also add `GitError::FetchFailed`, mirroring `PushFailed`, so a failure names
   the remote instead of surfacing as an opaque `Git2` passthrough, and raise
   the sync actor's failure log from `debug!` to `warn!` — a fetch failing on
   every tick should be visible, not swallowed.
   
   Fetch-only remains deliberate: the daemon advances remote-tracking refs and
   never fast-forwards local branches, since a user or worker may have one
   checked out. Documented in both places so it stays that way.
 ...
 - <csr-id-1e631090a894540fc9f913f7f1e9aab416347d3c/> enforce timeout, stop consuming worker budget
   Three defects in the subprocess-based triage path.
   
   1. No timeout. run_triage_subprocess called Command::output(), which blocks
      indefinitely. The actor has a single-slot queue and refuses sends while a
      handle is in flight, so one hung agent silently stopped all further triage
      for the daemon's lifetime -- no log, nothing in jig ps. Replaced with
      run_with_timeout(), which polls try_wait() and kills the child once it
      outruns triage.timeout_seconds. stdout/stderr are drained on their own
      threads: a child that fills a pipe buffer blocks on write, so polling for
      exit while holding unread pipes can deadlock a healthy process.
   
   2. timeout_seconds was never enforced. Its only reader was a stuck-entry sweep
      at the top of handle(), but handle() registers, runs and removes an entry
      within one blocking iteration on the actor thread, so the tracker is always
      empty when the sweep runs and the elapsed check could never fire. The config
      key was documented (wiki/_docs/init.md) but dead. The timeout is now applied
      directly to the subprocess, which is where it belongs, and the unreachable
      sweep is gone.
   
   3. Triage consumed the worker budget. The poll skipped entirely when
      max_concurrent_workers - existing_worktrees was 0, and capped issues by that
      budget. Triage creates no worktree, branch or mux window, so busy workers
      silently disabled triage. Removed.
   
   Also drops the vestigial worker_name from TriageEntry: it was formatted as
   "triage-<issue-id>" for a worker that never existed, and only fed log lines and
   the ps WORKER column. TriageEntry now carries the agent model instead, and the
   ps triage table shows MODEL -- the column now describes something real.
   
   Tests cover the kill path (verified it returns promptly rather than waiting out
   a sleep 30), exit-status and stderr capture, a missing binary, and a child
   pushing 2MB through a pipe, which would deadlock without the drain threads.
 - <csr-id-9ffdb3641c5c856dc71ce84734854cf7f732a8e3/> stop --plain output emitting ANSI escape codes
   Every table builder in ps/render.rs called enforce_styling(), which makes
   comfy-table emit ANSI unconditionally, and the grouped renderers wrote raw
   \x1B[1m escapes for their repo headers. The is_plain() checks only stripped the
   "TRIAGES" section label, so `jig ps --plain` still shipped escape codes in the
   table body and repo headers -- contradicting PATTERNS.md ("all helpers respect
   --plain") and defeating the flag's purpose for scripting.
   
   Collapse the four duplicated preset/styling blocks into one ps_table() helper
   that owns the policy: disable styling outright in plain mode via force_no_tty(),
   and force it on otherwise. Forcing is required rather than relying on
   comfy-table's auto-detection because ps writes to stderr while that detection
   inspects stdout. Raw escapes are replaced with ui::bold(), which already honors
   plain mode, and the duplicated indent logic becomes indent_lines().
...
### Refactor
 ...
 - <csr-id-f07be5833d3a03f33a2d4c5b00770f3f1429e2b9/> one Sandbox in jig-cli; ship Repo::init
   The fixture lived in jig-core only because that was the one place both
   crates could see it — but temp config and runtime roots are jig-cli's
   concern, not a git library's. And it took two types (`Fixture` for the
   dirs, `Sandbox` for running the binary) because jig-cli had no lib
   target for `tests/` to import.
   
   - `Repo::init(path)` joins the real API: init, on `main`. jig-core's git
     tests seed a commit through a `#[cfg(test)]` helper beside them, so the
     crate carries no test fixtures and no `test-support` feature.
   - jig-cli gets `src/lib.rs`; `main.rs` is now argument parsing, logging
     setup and dispatch over it. Integration tests can reach internals
     instead of only the CLI surface.
   - One `jig_cli::test_support::Sandbox` replaces both types: temp roots,
     any number of repos, `paths()` for in-process tests, `jig()` /
     `start_daemon()` for the binary. `tests/common/` is gone. It is behind
     a `test-support` feature that only the self dev-dependency enables, so
     `cargo tree -e normal` shows neither tempfile nor assert_cmd.
 - <csr-id-f84a032d990679a7eb5f0d2f15f83c33e7d574c6/> parse pr_url once, type spawning, split out WorkerState
   Three of a kind: values that had a real type were being carried as text.
   
   - `WorkerState.pr_url` is a `Url`, parsed once in the reducer, and the
     `parsed_pr_url` cache beside it is gone — one value, one truth. The
     event log still records the string it came from. Notification events
     take text, so the monitor renders it there.
   - `DaemonStatus.spawning` is `Vec<Branch>`; the spawn actor already had
     a `Branch` from `issue.branch()` and flattened it to `String`.
   - `WorkerState` and its impls move to `worker/events/worker_state.rs`,
     leaving `reducer.rs` to the reduction. Easier to find, and the file is
     no longer two things at once.
   
   `url` gains the `serde` feature so `Url` crosses the wire as itself.
 - <csr-id-3163d1e3a782446f37b332ce62fb7172b303895d/> keep Branch typed on the wire; stop sending parsed_pr_url
   `Branch` already derives Serialize/Deserialize, so the snapshot's
   `String` round-trip bought nothing — and stringly-typed branches are
   what produced the folder-vs-branch mixup in KRO-217.
   
   `parsed_pr_url` is `pr_url` after parsing, so sending both put the same
   value on the wire twice, with nothing keeping them agreed.
   `to_display_state` re-parses instead.
 - <csr-id-cd9f8eccce5c9dff2dd5ca672012b20b71dbe7e9/> rename JigDirs to AppPaths
   Most of its methods return file paths (socket, pid file, config file,
   registry), not directories, and the name should say what it's for: jig's
   own files, as opposed to the user's repos and worktrees. Bindings go from
   `dirs` to `paths` (`ctx.paths`, `paths.socket()`); the `dirs` crate is
   untouched. No behavior change.
 - <csr-id-e9553ddefce26022e847b6b93ea07a620166986b/> commands choose their log sink; daemon records its log
   Every invocation used to send tracing to a file under state/logs, so
   one-off commands' warnings went where nobody looked — and `main` had to
   guess, before running anything, whether the process would host the
   daemon, because `jig daemon logs` identified the daemon's log by a
   `-daemon.log` name and picked the newest. With `ps --watch` able to be
   either host or client, that guess needed a socket ping before tracing
   even started (`runs_daemon_loop`).
 - <csr-id-7605e72499f839d0c7451ff32c226fb04e35de27/> resolve JigDirs once in main and pass it down
   Every jig path came from free functions in context/paths.rs that read
   XDG_CONFIG_HOME / XDG_RUNTIME_DIR on each call, so unit tests could only
   point code at temp dirs by mutating the process environment.
   
   `JigDirs` (config root + runtime root, with every path jig uses as a
   method) is now resolved from the environment exactly once, in `main`,
   and passed down:
   - `Op::build_context(&self, dirs)`; the command macro and the nested
     groups (pr, issues, daemon) thread it through. Context structs carry
     `dirs`; repo-less commands take `JigDirs` as their context.
   - Config, RepoRegistry, NotificationQueue, the daemon lifecycle log,
     worker event logs, Worker's log-touching methods, PID file, IPC
     client/server, and the daemon (via TickContext) all take `&JigDirs`.
   - Path construction no longer does I/O, so event logs and the
     notification queue stop returning Result.
   
   Nothing below `main` reads an XDG variable any more. Unit tests build
   `JigDirs::from(&Fixture)` instead; the free-function paths.rs API is
   gone. Also: `worktree_event_status` no longer reloads the global config
   once per `jig ls` row, and `resolve_base_branch_for` takes the loaded
   config.
...
### Reverted
 ...
 - <csr-id-6b9cc83e736c02c2bf13b558b9a46a58ae373350/> drop nudge cap and wiki changes from this PR
   Out of scope — this PR only stops draft review comments counting as feedback.

### Style

 - <csr-id-4257260503d0abccc0af151a0885b400c2afba40/> use the ui helper for the incomplete-table warning
   Status messages go through `ui::` so they respect `--plain`, per
   docs/PATTERNS.md — and `daemon start` already reports the same timed-out
   pass that way.
...
### Test
 ...
 - <csr-id-fe632e9d3f0c85546e7d5924145285124f784196/> group the integration tests by area
   Nineteen flat files made it hard to see what was covered. Each area is
   now a directory with a `main.rs` declaring its modules — which cargo
   builds as one test binary, so there are 7 instead of 19:
   
       tests/cli/       attach, commit, notify, multi_repo, startup
       tests/context/   config_file, repo_config, log_tailer, single_repo
       tests/daemon/    ipc, pidfile, actors/prune
       tests/hooks/     install, uninstall, registry, handlers
       tests/notify/    queue, notifier
       jig-core: tests/git/{branches,repo}, tests/event_log.rs
   
   `tests/common/mod.rs` stays one copy, pulled into each area's `main.rs`
   with `#[path]`. Layout documented in docs/PATTERNS.md.
 - <csr-id-7c815ea8239799726d5342aea98d1a4af298476c/> move the Sandbox fixture out of src into tests/common
   With the filesystem tests out of `src/`, nothing in the library used the
   fixture any more — only the ten integration test files did. So it belongs
   beside them.
   
   `tests/common/mod.rs` owns `Sandbox` again, reaching jig's internals
   through the `jig_cli` lib rather than a `test-support` module inside it.
   That drops the `test-support` feature, the self dev-dependency that
   enabled it, and the optional tempfile/assert_cmd entries; both are plain
   dev-dependencies again. Neither crate's `src/` holds test setup now.
 - <csr-id-1016a46322a07ec855c4d95538ef505b0bb8a1b2/> move filesystem tests out of src and into tests/
   Unit tests are for pure logic; a test that makes directories, repos or
   config files is an integration test and now lives in `tests/`, reaching
   the code through the lib target added with the Sandbox.
   
   jig-cli gains tests/: config_file, context, daemon_pidfile, daemon_prune,
   git_hooks, hooks_install, hooks_registry, hooks_uninstall, log_tailer,
   notification_queue, notifier, repo_config. jig-core gains event_log,
   git_branches and git_repo, sharing a `seeded_repo` helper in
   tests/common. What stays inline is pure: reducers, path resolution, the
   wire round-trips, nudge gating.
   
   Six tests stay in src because they exercise a private helper
   (`exec_hook`, `expand_tilde`, `open_worktree`, `find_valid_start_point`,
   `pid_alive`) — better beside the code than widening visibility for a
   test. `Context::for_repo` is now pub, which it earned: build a context
   for a repo you already have.
   
   Also documents the rule in docs/PATTERNS.md.
 - <csr-id-d36240b99fa3e814a330cbcd643af868fa4a4e21/> Fixture with any number of repos; Sandbox wraps it
 - <csr-id-a7e8d7ebd60b48f485ce3e65317835e2ef90c64e/> one shared init_repo fixture for both crates
   Five tests built the same "git repo on main, identity set, signing off,
   one empty commit" by hand — three git2 copies in jig-core, and the git
   CLI in jig-cli's context tests and integration sandbox (one of the
   jig-core copies never renamed its branch to main).
 - <csr-id-e114ff3d1d9c051be617134cd3dd5f0cd8314122/> share one sandbox across the integration tests
   attach, commit_validate, notify and daemon_ipc each hand-rolled a temp
   config dir + `cargo_bin("jig")` + env. Only daemon_ipc also isolated
   XDG_RUNTIME_DIR, so the others could reach a developer's real daemon
   socket on Linux.
   
   `tests/common/mod.rs` now owns that setup: every `Sandbox` gets its own
   XDG_CONFIG_HOME and XDG_RUNTIME_DIR, passed to each `jig` it runs, and
   never touches the test process's own environment — so tests and the
   daemons they start run in parallel (daemon_ipc_tests: 12.1s serial,
   4.0s parallel).
   
   Also moves the global-dirs check out of a unit test that mutated
   XDG_CONFIG_HOME into `global_dirs_tests.rs`.
 - <csr-id-62bb3de01e9fa12b93c3ab8a624e7747fdf282e4/> cover the socket lifecycle end to end
   Each test gets its own `XDG_CONFIG_HOME` *and* `XDG_RUNTIME_DIR`, so the
   daemons they start cannot see each other or the developer's own.
   
   Covers the three claims in the issue: a second daemon fails with
   "already running (pid N)", `daemon status` answers over the socket, and
   `ps` works on both the IPC and fallback paths — plus that a clean exit
   clears the socket and PID file, and that the pair a SIGKILLed daemon
   leaves behind does not block the next start.

### Commit Statistics

<csr-read-only-do-not-edit/>

 - 137 commits contributed to the release.
 ...
 * **Uncategorized**
    - Bump version to 0.9.1 ([`0be92fc`](https://github.com/krondor-corp/jig/commit/0be92fcc3329a14892f0fce4aa49de59a4e3fbdf))
    - Merge pull request #84 from krondor-corp/fix/kro-217-pr-lookup-by-branch ([`1bb77cb`](https://github.com/krondor-corp/jig/commit/1bb77cbe7a8ca88d6df99661967298ce71942078))
    - Look up PRs by the checked-out branch (KRO-217) ([`612ea5e`](https://github.com/krondor-corp/jig/commit/612ea5e864a4dd5f23021d4158bca2cc09e911ad))
    - Merge pull request #80 from krondor-corp/release-automation ([`fa843ef`](https://github.com/krondor-corp/jig/commit/fa843ef6836776d5227fb093bf5b3eb1ee1a608e))
    - Bump jig-core v0.9.0, jig-cli v0.9.0 ([`bf6ae78`](https://github.com/krondor-corp/jig/commit/bf6ae78b98fa3cf83cb971a24fe2c28512aedb2f))
    - Bump version to 0.9.0 ([`af1abaf`](https://github.com/krondor-corp/jig/commit/af1abaf595b26f3d8dc08c61f352dee4f727c318))
    - Merge pull request #79 from krondor-corp/feat/daemon-install-as ([`8573472`](https://github.com/krondor-corp/jig/commit/8573472aa101b657f3a469b49b8aa387cac47892))
    - Jig daemon install --as <user> (and --env) ([`01ba214`](https://github.com/krondor-corp/jig/commit/01ba21482b0a5c28af9b8a1e572b03c9e6c5752a))
    - Merge pull request #77 from krondor-corp/feat/daemon-system-service ([`a741cff`](https://github.com/krondor-corp/jig/commit/a741cff474d97b2e41345aa6daf90f39499218dd))
    - Install as a system service that runs as you (Linux) ([`d6c579d`](https://github.com/krondor-corp/jig/commit/d6c579d770d77c7a5a2d4bea0c6799d2cfc0d94d))
    - Merge pull request #75 from krondor-corp/release-automation ([`5e8d440`](https://github.com/krondor-corp/jig/commit/5e8d440957117a64575fee54f1bc575c9cf036b4))
    - Bump jig-core v0.8.0, jig-cli v0.8.0 ([`2665515`](https://github.com/krondor-corp/jig/commit/26655155614d6f34c5aa248869c065fb495b24a3))
    - Bump version to 0.8.0 ([`6b4aa31`](https://github.com/krondor-corp/jig/commit/6b4aa31f5c42e77db93ad73ec7e3f40a798dc914))
    - Merge pull request #74 from krondor-corp/feat/daemon-service-install ([`370dfa4`](https://github.com/krondor-corp/jig/commit/370dfa4835cfc2fea92aef9951e66e3bf1d6e3ef))
    - Jig daemon install | uninstall (service plan phase 5) ([`1a4364c`](https://github.com/krondor-corp/jig/commit/1a4364ca9ab2b1f64e00f4e982c349f7dde0a992))
    - Merge pull request #73 from krondor-corp/release-automation ([`05bf2d7`](https://github.com/krondor-corp/jig/commit/05bf2d74221a17e0d2fd8c8f28100ef9f33d4cb2))
    - Bump jig-core v0.7.0, jig-cli v0.7.0 ([`99e39fa`](https://github.com/krondor-corp/jig/commit/99e39fa58fe4ff7d71c7c063e744805142040143))
    - Bump version to 0.7.0 ([`ef249cb`](https://github.com/krondor-corp/jig/commit/ef249cba46122dda853f603e5a7851a4e20cef43))
    - Merge pull request #70 from krondor-corp/revive/kro-213-socket-daemon ([`9047c6a`](https://github.com/krondor-corp/jig/commit/9047c6a0b310d4829ccb62703eb0451514ca8967))
    - Group the integration tests by area ([`fe632e9`](https://github.com/krondor-corp/jig/commit/fe632e9d3f0c85546e7d5924145285124f784196))
    - Move the Sandbox fixture out of src into tests/common ([`7c815ea`](https://github.com/krondor-corp/jig/commit/7c815ea8239799726d5342aea98d1a4af298476c))
    - Move filesystem tests out of src and into tests/ ([`1016a46`](https://github.com/krondor-corp/jig/commit/1016a46322a07ec855c4d95538ef505b0bb8a1b2))
    - One Sandbox in jig-cli; ship Repo::init ([`f07be58`](https://github.com/krondor-corp/jig/commit/f07be5833d3a03f33a2d4c5b00770f3f1429e2b9))
    - Parse pr_url once, type spawning, split out WorkerState ([`f84a032`](https://github.com/krondor-corp/jig/commit/f84a032d990679a7eb5f0d2f15f83c33e7d574c6))
    - Keep Branch typed on the wire; stop sending parsed_pr_url ([`3163d1e`](https://github.com/krondor-corp/jig/commit/3163d1e3a782446f37b332ce62fb7172b303895d))
    - Merge pull request #69 from krondor-corp/release-automation ([`c47d4c5`](https://github.com/krondor-corp/jig/commit/c47d4c5c09f775cba18cd74c5450e35bc4cafe49))
    - Bump jig-core v0.6.0, jig-cli v0.6.0 ([`70bbde7`](https://github.com/krondor-corp/jig/commit/70bbde7a3e8fb58f3842d9259587cf4380d2adcf))
    - Reapply "feat(daemon): bind the daemon to a unix socket (service plan phases 1-4)" ([`99e4dcb`](https://github.com/krondor-corp/jig/commit/99e4dcbb9e873277cd452e2b2b744bd00bddb55b))
    - Bump version to 0.6.0 ([`b03b2c1`](https://github.com/krondor-corp/jig/commit/b03b2c10f48ca83266267c53d0d24e51aa6944ec))
    - Merge pull request #68 from krondor-corp/revert-64-feature/kro-213-bind-the-daemon-to-a-unix-socket-daemon-service-plan-phases ([`6fbc865`](https://github.com/krondor-corp/jig/commit/6fbc865e417265e64404445e46bd1a906529e385))
    - Revert "feat(daemon): bind the daemon to a unix socket (service plan phases 1-4)" ([`a4487fd`](https://github.com/krondor-corp/jig/commit/a4487fd08152abe190ff8c3194f8172990bcb6c4))
    - Merge pull request #64 from krondor-corp/feature/kro-213-bind-the-daemon-to-a-unix-socket-daemon-service-plan-phases ([`10733bd`](https://github.com/krondor-corp/jig/commit/10733bdc46f91990b213be10c9671b19a48f84fc))
    - Rename JigDirs to AppPaths ([`cd9f8ec`](https://github.com/krondor-corp/jig/commit/cd9f8eccce5c9dff2dd5ca672012b20b71dbe7e9))
    - Commands choose their log sink; daemon records its log ([`e9553dd`](https://github.com/krondor-corp/jig/commit/e9553ddefce26022e847b6b93ea07a620166986b))
    - Resolve JigDirs once in main and pass it down ([`7605e72`](https://github.com/krondor-corp/jig/commit/7605e72499f839d0c7451ff32c226fb04e35de27))
    - Fixture with any number of repos; Sandbox wraps it ([`d36240b`](https://github.com/krondor-corp/jig/commit/d36240b99fa3e814a330cbcd643af868fa4a4e21))
    - One shared init_repo fixture for both crates ([`a7e8d7e`](https://github.com/krondor-corp/jig/commit/a7e8d7ebd60b48f485ce3e65317835e2ef90c64e))
    - Stop unit tests mutating the process environment ([`93435fb`](https://github.com/krondor-corp/jig/commit/93435fb70ed5e6d53babf57686828e0f19aa89c7))
    - Share one sandbox across the integration tests ([`e114ff3`](https://github.com/krondor-corp/jig/commit/e114ff3d1d9c051be617134cd3dd5f0cd8314122))
    - Drop nudge cap and wiki changes from this PR ([`6b9cc83`](https://github.com/krondor-corp/jig/commit/6b9cc83e736c02c2bf13b558b9a46a58ae373350))
    - Ignore draft review comments; cap nudges at 3 then notify ([`5112660`](https://github.com/krondor-corp/jig/commit/5112660e9ea51467100c51a3907b299ee0663469))
    - Use the ui helper for the incomplete-table warning ([`4257260`](https://github.com/krondor-corp/jig/commit/4257260503d0abccc0af151a0885b400c2afba40))
    - Say when a one-shot table may be incomplete ([`9c01e17`](https://github.com/krondor-corp/jig/commit/9c01e174376fb635e4e235d826fbeee93dbe55cb))
    - Make the --once monitor wait configurable ([`35f39e9`](https://github.com/krondor-corp/jig/commit/35f39e94ac04fb8118782a9d84a08c9471328915))
    - Don't let a watching client shadow the daemon's log ([`e389909`](https://github.com/krondor-corp/jig/commit/e389909ef79c2f595295c0fd5d63e3e85b39ecfe))
    - Merge pull request #63 from krondor-corp/feature/kro-218-daemon-swallows-github-errors-pr-tracking-can-fail-silently ([`89cfd6d`](https://github.com/krondor-corp/jig/commit/89cfd6dd7376522591fe16df137b79b91d6d9e6a))
    - Lock down the socket directory and stop the key reader ([`5730e49`](https://github.com/krondor-corp/jig/commit/5730e49273805cefa260837789cb7b52193e78d6))
    - Cover the socket lifecycle end to end ([`62bb3de`](https://github.com/krondor-corp/jig/commit/62bb3de01e9fa12b93c3ab8a624e7747fdf282e4))
    - Render from the running daemon, falling back in-process ([`163d30c`](https://github.com/krondor-corp/jig/commit/163d30cf9748f798dd646fc5064f41fd2a1e15ea))
    - Jig daemon start | stop | status over the socket ([`a93a375`](https://github.com/krondor-corp/jig/commit/a93a37535a7f959654b901b501eff250294cfee8))
    - Bind a unix socket and enforce a single instance ([`a59fb06`](https://github.com/krondor-corp/jig/commit/a59fb0634e2b80d116bb26e49e3637427287eeb5))
    - Wire types for IPC — serde derives and socket paths ([`d679797`](https://github.com/krondor-corp/jig/commit/d67979702998c1e9609794af82afdbe6834a1173))
    - Flag a failed GitHub check in the HEALTH column ([`0365d6c`](https://github.com/krondor-corp/jig/commit/0365d6ccd806f92a36b54a860b27438704843291))
    - Report GitHub failures instead of swallowing them ([`ed65673`](https://github.com/krondor-corp/jig/commit/ed65673abbccd09786a5a2f943ca0a8e3dbebedf))
    - Merge pull request #61 from krondor-corp/fix/ignore-foreign-worktrees ([`f81770d`](https://github.com/krondor-corp/jig/commit/f81770d78ace72e3fa6a7d24f8d827b235e1b7dc))
    - Claim `pending` before dispatch so actors can't wedge ([`1c4ae48`](https://github.com/krondor-corp/jig/commit/1c4ae48276f0f01787ede1d774112bb501dbe939))
    - Say when non-jig worktrees are skipped; --all shows them again ([`9575e09`](https://github.com/krondor-corp/jig/commit/9575e09791a3cc4a7c039e2720c518eb09c15345))
    - Keep actors alive when a request panics ([`46c884c`](https://github.com/krondor-corp/jig/commit/46c884c215ca734207a1970799208ee119e2e69c))
    - Merge pull request #57 from krondor-corp/feat/daemon-logs-status ([`34265e9`](https://github.com/krondor-corp/jig/commit/34265e9b98bb638d47730ba682a790025d5b3e87))
    - Add `jig daemon status` and `jig daemon logs` ([`b598304`](https://github.com/krondor-corp/jig/commit/b598304239ed82eff3c08dea9fea43e0ff6b9963))
    - Merge pull request #55 from krondor-corp/fix/daemon-fetch-auth ([`772c3bc`](https://github.com/krondor-corp/jig/commit/772c3bc18a17ce4526bce54a7ff7939122fccd0c))
    - Authenticate the background fetch ([`24dd999`](https://github.com/krondor-corp/jig/commit/24dd9999d13ad949a32286d5c0c20526d8563ac7))
    - Merge pull request #54 from krondor-corp/release-automation ([`87d0fbf`](https://github.com/krondor-corp/jig/commit/87d0fbf6f3e55d9c94a8839e0baa76aaecf0ff7f))
    - Bump jig-core v0.5.2, jig-cli v0.5.2 ([`f03dc0a`](https://github.com/krondor-corp/jig/commit/f03dc0a8a8f9ba17282a7d97f243f8422d54a515))
    ...
    - Merge pull request #49 from krondor-corp/fix/triage-timeout-and-budget ([`c0c6c46`](https://github.com/krondor-corp/jig/commit/c0c6c4613ab02677fd6e24464a7b0b671e5b83e1))
    - Enforce timeout, stop consuming worker budget ([`1e63109`](https://github.com/krondor-corp/jig/commit/1e631090a894540fc9f913f7f1e9aab416347d3c))
    - Stop --plain output emitting ANSI escape codes ([`9ffdb36`](https://github.com/krondor-corp/jig/commit/9ffdb3641c5c856dc71ce84734854cf7f732a8e3))
...
<csr-id-12c5ea42f34574dfa9afbd1aa2872752089af72f/>
<csr-id-0a996ced56110b176b3de348cbbd0f510b4873ac/>
<csr-id-e855492f12a87e96e3d2e4826058f5f5b717e45a/>
<csr-id-c3a3b156f83255f6feb0b0e4d5854d0af5edf4f9/>
<csr-id-38fe1e498e8bfbf3b920933d323c695bcb4cd982/>
<csr-id-42acb0015ead348c20e6243fc119da452f8f4b2d/>
<csr-id-b26c1ddd1c3117efa6d4f1fad18750d25b9ca149/>
<csr-id-4b9b47f59c01c76937277b471472bde50a0a0dfa/>
<csr-id-2946d279e3b72fd97ef198894a707b668295a1a6/>
